...
servername = "http://ytdownload.pythonanywhere.com/"
...
def get_download_link():
    ...
    if "youtube_url" in request.form and "itag" in request.form:
        ...
        yt = YouTube(ytlink)
        filePath = yt.streams.get_by_itag(itag).default_filename
        yt.streams.get_by_itag(itag).download("files")
        fileId = randomString(20, 36)
        query = conn.execute(f'''
        INSERT INTO `data` (`file_id`, `file_path`) VALUES ('{fileId}', '{filePath}')
        ''')
        conn.commit()
        if query:
            dataToSend = {
...
def download(file_id):
    ...
    if data == None:
        ...
        ''')
        conn.commit()
        return send_file(return_data, mimetype="application/octet-stream", download_name=data[2], as_attachment=True)
